...
| 2.3 | **DONE** - A checkout without the art pack says so. `Cache.CheckAssets` looks for `assets/textures/sets/` once during `Engine.Initialize` and logs a `<W>` naming the folder, the itch.io download and install-guide step 6; it leaves the answer on `Cache.assets_missing`, which `/get_assets_status` hands to the main menu as a banner across the top. Nothing fails without the folder - every card is simply drawn as a stand-in - so the old symptom was a game that looked like its art was grey on purpose. The check is one `isdir` at start-up, and the menu re-reads the recorded answer rather than the disk. | P1 | S |
| 2.4 | **DONE** - A port already in use is one line, and usually not even that. `WebDeviceManager` walks forward from the asked-for port (`port_fallback_tries`, 10 by default) and serves on the first free one, saying which it took - a second copy of the game now just opens on 2346. When the whole range is taken it raises `StartupError` (`engine/startup_error.py`), which `Engine.Initialize` prints as a single `<E>` naming the addresses tried and `-port`, instead of an assert. The second traceback is gone too: `Engine.SaveCrash` has no scene to save before `Engine.game` is assigned, so it no longer reaches for one and buries the real error. | P2 | S |
